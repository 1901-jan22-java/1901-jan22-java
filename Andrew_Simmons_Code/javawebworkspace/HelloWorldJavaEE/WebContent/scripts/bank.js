...
function logIn(){
    ...
    xhr.open("GET", `http://localhost:8085/users?username=${username}`);
    xhr.send();
}
...
function displayUserAccounts(user){
    ...
    xhr.open("GET", `http://localhost:8085/accounts?userId=${user.id}`);
    xhr.send();
}
...
function addUser(){
    ...
    xhr.open("POST", "http://localhost:8085/users");
...
function isUnique(){
    ...
    xhr.open("GET", `http://localhost:8085/users?username=${uname}`);
    xhr.send();
}