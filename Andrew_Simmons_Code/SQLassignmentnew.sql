...
select * from Ablum ORDER BY title;
...
--task insert two new records into Employeee Table

select * from employee;

INSERT INTO employee ( genreid, firstname)
VALUES (26, 'Robot Reggae');

--task insert two new records  Customer Table
...
--2.7 DELETE
-- Constraint may be involved
DELETE FROM invoiceline
WHERE invoiceid IN (SELECT invoiceid FROM invoice
                   WHERE customerid IN (Select customerid FROM customer
                                        WHERE firstname = 'Robert' AND lastname = 'Walter'));

-- since no foreign refernece it can be deleted without violating anything
...
SELECT TO_CHAR
    (SYSDATE, 'MM-DD-YYYY HH24:MI:SS')
     ...
     -- function that returns the length of media type from media type table
