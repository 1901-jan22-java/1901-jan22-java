...
public class PrimeArrayList {	
	...
	public static boolean checkIfPrime(int num) {
        ...
        for (int i = 2; i <= Math.sqrt(num); i++) { // A prime number is any number which cannot be divided evenly by any number other than 1 and itself.
            remainder = num % i; // Using modulus operator to check for a remainder, indicating a number is evenly divisible by i
            if (remainder == 0) {
                return false;
            }
        }
