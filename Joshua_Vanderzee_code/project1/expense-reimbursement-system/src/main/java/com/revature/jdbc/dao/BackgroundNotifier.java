...
public class BackgroundNotifier implements Runnable {
	...
	public void run() {
		...
		while (true)
		{
			...
			if (ErsFactory.getLoginAtempt() >= 5)
			{
				if (!alert)
				{
					logger.info("Invalid Login Atempts detected. Contacting " + ErsFactory.getEmailAlert());
					System.out.println("Some email implementation code would be run");
					while (ErsFactory.getEmailAlert().equals("")) {
						try {
							Thread.sleep(1000);
					...
					System.out.println(ErsFactory.getEmailAlert());
